	...
	for f in myfiles:
		...
		if f == 'general':
			...
			for g in general_list:
				...
				np.array(X1_total.extend(X))
				np.array(y1_total.append(0))
				#print y_total
				#print("predict:"+str(predicted_genre))
				# Visualize result using SVD
		elif f == 'queenless':
			general_path = "./testSVD/queenless/"
			general_list = os.listdir(general_path)
			total_general_num = len(general_list)
			general_w_queenloss_num = 0;
			general_w_virus_num = 0;
			for g in general_list:
				...
				test_file = os.path.join(general_path,g)
				X,y = test_model_on_single_file(test_file)
				np.array(X2_total.extend(X))				
				np.array(y2_total.append(1))
				#print("predict:"+str(predicted_genre))
				# Visualize result using SVD
		elif f == 'virus':
			general_path = "./testSVD/virus/"
			general_list = os.listdir(general_path)
			total_general_num = len(general_list)
			general_w_queenloss_num = 0;
			general_w_virus_num = 0;
			for g in general_list:
				...
				test_file = os.path.join(general_path,g)
				X,y = test_model_on_single_file(test_file)
				np.array(X3_total.extend(X))
				
				np.array(y3_total.append(2))
				#print("predict:"+str(predicted_genre))
				# Visualize result using SVD
	#print X_total.shape
	#print y_total.shape
	svd = TruncatedSVD(n_components=2) 
	X_reduced = svd.fit_transform(X1_total)
	# Initialize scatter plot with x and y axis values
	plt.scatter(X_reduced[:, 0], X_reduced[:, 1], c='green', s=25,label='normal')
	
	svd = TruncatedSVD(n_components=2) 
	X_reduced = svd.fit_transform(X2_total)
	# Initialize scatter plot with x and y axis values
	plt.scatter(X_reduced[:, 0], X_reduced[:, 1], c='red', s=25,label='queenless')
	
	svd = TruncatedSVD(n_components=2) 
	X_reduced = svd.fit_transform(X3_total)
	# Initialize scatter plot with x and y axis values
	plt.scatter(X_reduced[:, 0], X_reduced[:, 1], c='blue', s=25,label='virus')
